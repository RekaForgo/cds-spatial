...
mun <- st_transform(mun_sf, crs = 25832) # 32632
...
head(elections_data)
tail(unique(elections_data$Name), 10)

# Join the election data with municipality polygons
sum(mun$NAME_2%in%unique(elections_data$Name))

elections <- mun %>% 
  full_join(elections_data, by = c("NAME_2"="Name"))
...
elections %>% 
  ...
  tm_polygons("pct_vote2011",
              fill.legend = tm_legend("Percentage of vote in 2011", orientation = "portrait", position = tm_pos_out("RIGHT", "center")))
...
elections %>% 
  ...
  tm_polygons("pct_vote2015",
              fill.legend = tm_legend("Percentage of vote in 2015", orientation = "portrait", position = tm_pos_out("RIGHT", "center")))
...
elections %>% 
  ...
  tm_polygons("pct_vote2019",
              fill.legend = tm_legend("Percentage of vote in 2019", orientation = "portrait", position = tm_pos_out("RIGHT", "center")))
...
plot(DF$pct_vote2015, st_area(DF$geometry, byid = TRUE)/1000000, xlab = "Vote %", ylab = "Area (km2)", main = "Dansk Folkeparti fraction per municipality area")
...
plot(DF2015$pct_vote2015, st_area(DF2015$geometry, byid = TRUE)/1000000, xlab = "Vote %", ylab = "Area (km2)", main = "Dansk Folkeparti fraction per municipality area (corrected)")
...
plot(SD$pct_vote2015, st_area(SD , byid = TRUE)) # reality
plot(SD2015$pct_vote2015, st_area(SD2015, byid = TRUE)) # cartogram


# Make a adjusted map of the 2015 SD and DF voters
plot(SD["pct_vote2015"],
   #  col="pink",
     main = "% of Social Democrat votes across DK in 2015")

plot(SD2015["pct_vote2015"],
     #col="pink",
     main = "% of Social Democrat votes across DK in 2015")

plot(DF["pct_vote2015"],
   #  col="pink",
     main = "% of Danske Folkeparti votes across DK in 2015")

plot(DF2015["pct_vote2015"],
    # col="lightblue",
     main = "% of Danske Folkeparti votes across DK in 2015 (corrected sizes)")
```
...
mun_drama <- st_simplify(mun, dTolerance = 500)
