...
The forests layer contains 32 features and they are polygons. 
The parks layer contains 47 features and they are polygons. 
The playgrounds layer contains 51 features and they are points. 
The shelters layer contains 19 features and they are points.

2. What is the CRS value in these objects?
The CRS value shows a projected CRS with the UTM zone 32N and the ETRS89 datum.
...
1. What are the dimensions (number of columns and rows) of the `elevation` raster?  396, 876, 346896

2. What is the resolution of `aarhus` orthophoto? How many layers does it contain and what do they represent?

The resolution is 1:1 - each pixel represents 1m x 1m on the ground. It contains 5 layers (red, green, blue and alpha (transparency)). 
...
The units are in meters. The elevation data is in a projected CRS (UTM) and the orthophoto is in an unprojected CRS (WGS84). In the projected CRS, the units are in meters, while in the unprojected CRS, the units are in degrees of latitude and longitude. This is why the extents and number of cells for the two datasets are different.
...
crs_2 <- "+proj=utm +zone=32 +datum=ETRS89 +units=m +no_defs"
